...
//mergeSort (take inputArray) 
var mergeSort = function(inputArray) {
  // edge case = arrays that are empty or have one item
  ...
  // split into left and right arrays
    ...
    //call merge with the recursive call of mergeSort(left) and mergeSort(right) 
    return merge(mergeSort(left), mergeSort(right));
}

//merge (takes leftArray, rightArray as input)
var merge = function(left, right) {
  // create result array
  var results = [];

  // while there is something remaining in right or left array 
  while(left.length && right.length) {
    // do the sorting 
    // if left element is < right element 
    if(left[0] < right[0]) {
      // push the left element into results  --> left[0]
      results.push(left[0]);
      // shift the left element out of array
      left.shift();
    } else { 
      // push the right element into results --> right[0]
      results.push(right[0]);
      // shift the right element out of array
      right.shift();
    }
  }

  // if there is an element remaining in right or left array // push onto results array
  if(left.length) {
    results.push(left[0]);
