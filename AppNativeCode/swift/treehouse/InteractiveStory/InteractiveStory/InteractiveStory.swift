...
// this is our story model for each page // specifiy String for rawValue
enum Story: String {
    case ReturnTrip
    case TouchDown
    case Homeward
...
// each page has 3 things: Image, text, sound effect 
// extensions : allow you to group types of functionality and ... ?

// A page must have a story, optionally have a choice or two on next step

extension Story {
    // computed property: set of braces to define values returned --> images names match cases in enum
    var artwork: UIImage {
...
class Page {
    ...
    // type alias - alt name for type 
    typealias Choice = (title: String , page: Page) // Choice is scoped to the page class
    
    var firstChoice: Choice?
    var secondChoice: Choice?
    
    init(story: Story) { // have to init with a class
        self.story = story
    }
}

extension Page {
    // we can write a func with the same name as another but diff arguments
    func addChoice(title: String, story: Story) -> Page {
        // this sets up page for us and then passes the title and page to the second addChoice(f) based on the story passed in
        let page = Page(story: story)
        return addChoice(title, page: page)
    }
    func addChoice(title: String, page: Page) -> Page {
        switch (firstChoice, secondChoice) { // swictching on both possible values
        // if first choice and second choice have a value of .Some this means they are not nil
        case (.Some, .Some):  break  // so if both have a page assigned to them, break, and just return the page
...
struct Adventure {
    // our data is a tree structure : one page can go to 2 pages and each of those to two more etc
    static var storyAdventure: Page { // static - read only - cannot change
        ...
        let rover = touchdown.addChoice("Explore Rover", story: .Rover)
        ...
        let cave = rover.addChoice("Explore Coordinates", story: .Cave)
